    ...
    internal class MNK
    {
            ...
            protected override uint Invoke(uint actionID, uint lastComboActionID, float comboTime, byte level)
            {
                ...
                if (actionID is Bootshine or LeapingOpo)
                {
                    ...
                    if (inCombat)
                    {
                        ...
                        if (IsEnabled(CustomComboPreset.MNK_STUseBuffs) && canWeave)
                        {
                            ...
                            if (level >= Levels.RiddleOfFire
                                && ((!IsOnCooldown(RiddleOfFire) && CombatEngageDuration().TotalSeconds > 30) 
                                || (Gauge.OpoOpoFury > 0 && Gauge.CoeurlFury > 0 && Gauge.RaptorFury > 0 && CombatEngageDuration().TotalSeconds <= 30)))
                            {
                                return RiddleOfFire;
                            }
                    ...
                    if (inCombat)
                    {
                        ...
                        {
                            return Gauge.OpoOpoFury == 0 && ActionReady(DragonKick) || ActionReady(OriginalHook(Bootshine)) ? OriginalHook(DragonKick) : OriginalHook(Bootshine);
                        }

                        if (IsEnabled(CustomComboPreset.MNK_STUsePerfectBalance))
                        {
                            // Masterful Blitz
                            if (level >= Levels.MasterfulBlitz && !HasEffect(Buffs.PerfectBalance) && HasEffect(Buffs.RiddleOfFire) && OriginalHook(MasterfulBlitz) != MasterfulBlitz && ActionReady(OriginalHook(MasterfulBlitz)))
                            {
                            ...
                            if (HasEffect(Buffs.PerfectBalance))
                            {
                                ...
                                if (!solarNadi && !bothNadisOpen)
                                {
                                    if (coeurlChakra == 0)
                                    {
                                        return Gauge.CoeurlFury == 0 && (ActionReady(Demolish) || ActionReady(OriginalHook(SnapPunch))) ? OriginalHook(Demolish) : OriginalHook(SnapPunch);
                                    }
                                    else if (raptorChakra == 0)
                                    {
                                        return Gauge.RaptorFury == 0 && (ActionReady(TwinSnakes) || ActionReady(OriginalHook(TrueStrike))) ? OriginalHook(TwinSnakes) : OriginalHook(TrueStrike);
                                    }
        ...
        public static uint DetermineCoreAbility(uint actionId, bool useTrueNorthIfEnabled = true)
        {
            if (CustomComboFunctions.HasEffect(Buffs.OpoOpoForm) || CustomComboFunctions.HasEffect(Buffs.FormlessFist) 
                && (CustomComboFunctions.ActionReady(DragonKick) || CustomComboFunctions.ActionReady(CustomComboFunctions.OriginalHook(Bootshine))))
            {
                if (Gauge.OpoOpoFury == 0)
                {
            ...
            if (CustomComboFunctions.HasEffect(Buffs.RaptorForm) && (CustomComboFunctions.ActionReady(TwinSnakes) || CustomComboFunctions.ActionReady(CustomComboFunctions.OriginalHook(TrueStrike))))
            {
                if (Gauge.RaptorFury == 0)
                {
            ...
            if (CustomComboFunctions.HasEffect(Buffs.CoeurlForm) && (CustomComboFunctions.ActionReady(Demolish) || CustomComboFunctions.ActionReady(CustomComboFunctions.OriginalHook(SnapPunch))))
            {
                if (Gauge.CoeurlFury == 0)
                {
