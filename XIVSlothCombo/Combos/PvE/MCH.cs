                    ...
                    ((LevelChecked(FullMetalField) && !WasLastWeaponskill(FullMetalField)) || !LevelChecked(FullMetalField)))
                {
                    ...
                    if (BSUsed > 1)
                    {
                        if (LevelChecked(BarrelStabilizer) && LevelChecked(Excavator))
                        {
                            ...
                            if (gauge.Battery == 100 && GetCooldownRemainingTime(BarrelStabilizer) is > 90 or <= 5)
                                return true;

                            if (gauge.Battery >= 50 && GetCooldownRemainingTime(BarrelStabilizer) is > 80)
                                return true;

                            if (gauge.Battery >= 50 && GetCooldownRemainingTime(Chainsaw) > 50 && GetCooldownRemainingTime(BarrelStabilizer) is < 60)
                                return true;
                        }
                        else
                        {
