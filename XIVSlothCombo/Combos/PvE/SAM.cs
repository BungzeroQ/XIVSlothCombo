    ...
    internal class SAM
    {
        ...
        internal class SAM_ST_AdvancedMode : CustomCombo
        {
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                ...
                float meikyostacks = GetBuffStacks(Buffs.MeikyoShisui);
                float doTDebuffRamainingTime = FindTargetEffect(Debuffs.Higanbana)?.RemainingTime ?? 0f;
                        ...
                        HasEffect(Buffs.Fugetsu) && HasEffect(Buffs.Fuka))
                    {
                        ...
                        if (IsEnabled(CustomComboPreset.SAM_ST_CDs_Iaijutsu) && LevelChecked(Iaijutsu))
                        {
                            ...
                            if ((!IsEnabled(CustomComboPreset.SAM_ST_CDs_Iaijutsu_Movement) ||
                                ...
                                ((oneSeal && doTDebuffRamainingTime < 10 && enemyHP > HiganbanaThreshold && !LevelChecked(Traits.EnhancedMeikyoShishui2)) ||
                                (oneSeal && doTDebuffRamainingTime < 0.1 && enemyHP > HiganbanaThreshold && LevelChecked(Traits.EnhancedMeikyoShishui2) && !HasEffect(Buffs.Tendo) && WasLastWeaponskill(Gekko)) ||
                                (twoSeal && !LevelChecked(MidareSetsugekka)) ||
                                (threeSeal && LevelChecked(MidareSetsugekka)) ||
                                (threeSeal && LevelChecked(TendoSetsugekka) && !HasEffect(Buffs.TsubameGaeshiReady))))
                                return OriginalHook(Iaijutsu);
